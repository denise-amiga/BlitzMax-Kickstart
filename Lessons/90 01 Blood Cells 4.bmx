...
Global gameDone = 0
Global passCount= 0, level = 0, totalScore = 0
...
Type TPlayArea
	...
	Function UpdateBorders()
		...
		' Green gate at bottom of screen
		SetColor( 0, 128, 0)
		...
		' Draw a red cell for each passCount on this level
		For Local i = 1 To passCount
			SetColor( 128, 0, 0)
			Local w = basicWidth
			...
			EndIf	
		Next
		' Green ticks for each level
		For Local i = 1 To level
			SetColor 0, 128, 0
			DrawRect( x1 + 15*(i-1), y1_border - 20, 10, 20)
		Next
	End Function
...
Function ShowInstructions()
	Repeat
		...
		TDrawText.Draw " to top the screen in good health"
		TDrawText.Draw "Avoid the blue virus cells "
		TDrawText.Draw " they will reduce your health "
		TDrawText.Draw "Perfect health earns 10 points "
...
Type TRedCell
	...
	Method ChecklockMove()
		If y <= TPlayArea.y1_borderBottom Then 
			If lockMove = False Then
				...
				totalScore = totalScore + 10.0 * (health / 255.0)
				Init()
				lockMove = True
			EndIf
		EndIf	
	...
	Method CheckCheats()
		If KeyHit(KEY_F) Then speed = speed + 1
		If KeyHit(KEY_S) Then passCount = passCount + 1
		If KeyHit(KEY_L) Then level = level + 1
	End Method
...
Type TVirus
	...
	Method Init()
		x = TPlayArea.x1
		y = Rand(TPlayArea.y1, TPlayArea.y2 - VIRUS_HEIGHT )
		w = TPlayArea.basicWidth
		h = VIRUS_HEIGHT 
		speedx = Rand(1,10)
		speedy = 0
